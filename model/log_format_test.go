...
func TestFormatUserLogsStripsStreamStatus(t *testing.T) {
	other := common.MapToJsonStr(map[string]interface{}{
		"model_price": 0.004,
	...
	parsed, err := common.StrToMap(logs[0].Other)
	require.NoError(t, err)
	assert.NotContains(t, parsed, "stream_status")
	assert.Equal(t, 0.004, parsed["model_price"])

	adminLogs := []*Log{{Other: other}}
	FormatAdminLogs(adminLogs)
	adminParsed, err := common.StrToMap(adminLogs[0].Other)
	require.NoError(t, err)
	assert.Contains(t, adminParsed, "stream_status")
}
