...
// object for non-admin viewers. No-op when the clamp is nil (the common case:
// no saturation happened).
...
// AppendRelayLogAdminInfo records relay routing diagnostics in the admin-only
// scope shared by successful and failed request logs.
func AppendRelayLogAdminInfo(ctx *gin.Context, other *model.LogOther) {
...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	AppendRelayLogAdminInfo(ctx, other)
...
func GenerateMjOtherInfo(relayInfo *relaycommon.RelayInfo, priceData types.PriceData) *model.LogOther {
	other := model.NewLogOther()
